...
local Settings = {
    ...
    MapESP = false,
    BaseESP = false,
    SpeedValue = 16, -- Normal walk speed
    DrawDistance = 999999,
    MenuToggleKey = Enum.KeyCode.LeftControl,
}
...
local function DisableAntiCheatDetection()
    -- Disable network replicator detection
    local function BlockDetection()
        pcall(function()
            -- Block teleport detection scripts
    ...
    -- Run detection blocker every frame
...
local TPMethod = 1 -- Rotate between methods for detection avoidance
...
local function AdvancedTeleport(targetPos, smooth)
    ...
    -- Method 1: Velocity + Physics Manipulation
    local function MethodVelocity()
        pcall(function()
            if not HumanoidRootPart then return end
            
            -- Add BodyVelocity for smooth transition
    ...
    -- Method 2: Humanoid MoveTo + CFrame Blend
    local function MethodMoveTo()
        pcall(function()
            ...
            for i = 1, steps do
                ...
                -- Humanoid move (natural, less detectable)
    ...
    -- Method 3: Direct CFrame + Anchor Bypass
    local function MethodCFrame()
        pcall(function()
            if not HumanoidRootPart then return end
            
            -- Temporarily modify physics
    ...
    -- Execute method + randomize for next time
    if TPMethod == 1 then
        MethodVelocity()
    ...
    TPMethod = TPMethod % 3 + 1 -- Rotate method
end
...
local function FindMyBase()
    for _, obj in pairs(Workspace:GetDescendants()) do
        if obj:IsA("Model") then
            if obj.Name:lower():find("base") and obj:FindFirstChild("Humanoid") == nil then
...
-- ============================================
-- 🧠 DETECT BRAINROT IN HAND (Holding)
-- ============================================
...
local StealCooldown = 1 -- 1 second cooldown
...
local function InstantSteal()
    ...
    local basePos = myBase:FindFirstChild("HumanoidRootPart") and myBase.HumanoidRootPart.Position or 
                    (myBase.PrimaryPart and myBase.PrimaryPart.Position) or
                    myBase.Position
    
    LastStealTime = currentTime
    
    -- TP to base with anti-cheat bypass
...
local function FindMostExpensiveBrainrot()
    ...
    for _, obj in pairs(Workspace:GetDescendants()) do
        if obj:IsA("Model") and (obj.Name:lower():find("brainrot") or 
            obj.Name:lower():find("meowl") or obj.Name:lower():find("elephant") or 
            obj.Name:lower():find("divine")) then
            
            -- Try to get price from name or GUI
...
print("📌 Click ☰ button (top-left) to toggle menu")
